#!/bin/bash
set -e
# Program to build and package macOS monitoring-agent
# Monitoring Agent package generator
...
SERVICE_PATH="/Library/LaunchDaemons/com.monitoring.agent.plist"
...
function clean_and_exit() {
    ...
    if [ -z "$BRANCH_TAG" ]; then
        make -C $MONITORING_PATH/src clean clean-deps
    fi
...
function sign_binaries() {
    if [ ! -z "${KEYCHAIN}" ] && [ ! -z "${CERT_APPLICATION_ID}" ] ; then
        ...
        # Sign every single binary in Monitoring Agent's installation. This also includes library files.
...
function prepare_building_folder() {
    ...
    if [ -d "$CURRENT_PATH/monitoring-agent" ]; then
        ...
        echo "\nThe monitoring agent building directory is present on this machine."
        echo "Removing it from the system."

        rm -rf $CURRENT_PATH/monitoring-agent
    fi

    munkipkg --create --json $CURRENT_PATH/monitoring-agent

    cp -f $build_info_file $CURRENT_PATH/monitoring-agent/

    sed -i '' "s|VERSION|$version|g" $CURRENT_PATH/monitoring-agent/$(basename $build_info_file)
    sed -i '' "s|PACKAGE_NAME|$pkg_final_name|g" $CURRENT_PATH/monitoring-agent/$(basename $build_info_file)

    cp $preinstall_script $CURRENT_PATH/monitoring-agent/scripts/preinstall
    cp $postinstall_script $CURRENT_PATH/monitoring-agent/scripts/postinstall

    sed -i '' "s|PACKAGE_ARCH|$ARCH|g" $CURRENT_PATH/monitoring-agent/scripts/preinstall
...
function build_package() {
    ...
    if [ -n "$BRANCH_TAG" ]; then
        ...
        MONITORING_PATH="${SOURCES_DIRECTORY}/monitoring-agent"
        git clone --depth=1 -b ${BRANCH_TAG} ${MONITORING_SOURCE_REPOSITORY} "${MONITORING_PATH}"
    else
        MONITORING_PATH="${CURRENT_PATH}/../.."
    fi
    short_commit_hash="$(cd "${MONITORING_PATH}" && git rev-parse --short=7 HEAD)"

    export CONFIG="${MONITORING_PATH}/etc/preloaded-vars.conf"
    MONITORING_PACKAGES_PATH="${MONITORING_PATH}/packages/macos"
    ENTITLEMENTS_PATH="${MONITORING_PACKAGES_PATH}/entitlements.plist"

    VERSION="$(awk -F'"' '/"version"[ \t]*:/ {print $4}' $MONITORING_PATH/VERSION.json)"

    # Define output package name
    if [ $IS_STAGE == "no" ]; then
        pkg_name="monitoring-agent_${VERSION}-${REVISION}_${ARCH}_${short_commit_hash}"
    else
        pkg_name="monitoring-agent-${VERSION}-${REVISION}.${ARCH}"
    fi

    if [ -d "${INSTALLATION_PATH}" ]; then

        echo "\nThe monitoring agent is already installed on this machine."
        echo "Removing it from the system."
    ...
    # create package
    if munkipkg $CURRENT_PATH/monitoring-agent ; then
        echo "The monitoring agent package for macOS has been successfully built."
        mv $CURRENT_PATH/monitoring-agent/build/* $DESTINATION/
        symbols_pkg_name="monitoring-agent-debug-symbols-${VERSION}-${REVISION}.${ARCH}-macos"
        cp -R "${MONITORING_PATH}/src/symbols"  "${DESTINATION}"
