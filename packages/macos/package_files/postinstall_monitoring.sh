...
# Modified by Monitoring Solutions Inc. (https://www.monitoring-solutions.com)
...
if [ -z "${upgrade}" ]; then
    echo "Generating Monitoring Agent configuration for a fresh installation."
    ...
    if [ -f "${INSTALLATION_SCRIPTS_DIR}/gen_ossec.sh" ]; then
        ...
        chown root:monitoring ${DIR}/etc/ossec.conf
...
# Register and configure agent if Monitoring environment variables are defined
if [ -z "${upgrade}" ]; then
...
if [ -n "${upgrade}" ] && [ -n "${restart}" ]; then
    ...
    launchctl bootstrap system /Library/LaunchDaemons/com.monitoring.agent.plist
fi
