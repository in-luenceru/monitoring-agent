...
function Set-Schedule {
    ...
    try {
        ...
        if ($IntervalHours -ge 168) {
            ...
            # For interval-based triggers, need RepetitionDuration for indefinite repeats
            $trigger = New-ScheduledTaskTrigger -Once -At (Get-Date).AddMinutes(5) -RepetitionInterval (New-TimeSpan -Hours $IntervalHours) -RepetitionDuration ([TimeSpan]::MaxValue)
        }
