...
function Install-AutoStartupTasks {
    Write-Log "Installing unified auto-startup tasks..." "INFO"
    ...
    Add-Content $LogFile "[$timestamp] [INFO] Starting unified auto-startup tasks installation" -ErrorAction SilentlyContinue
    ...
    try {
        ...
        # Create the enhanced startup task using unified auto-start script
        Write-Log "Creating unified agent startup task..." "INFO"
        Add-Content $LogFile "[$timestamp] [INFO] Creating unified agent startup task: $Script:TaskName" -ErrorAction SilentlyContinue
        
        # Check for unified auto-start script first, fall back to monitoring-only if not available
        $unifiedAutoStartScript = Join-Path $Script:AgentPath "UnifiedAutoStart.ps1"
        $monitoringAutoStartScript = Join-Path $Script:AgentPath "MonitoringAgentAutoStart.ps1"
        
        $autoStartScript = $unifiedAutoStartScript
        if (!(Test-Path $unifiedAutoStartScript)) {
            Write-Log "Unified auto-start script not found, using monitoring-only version" "WARN"
            $autoStartScript = $monitoringAutoStartScript
            if (!(Test-Path $monitoringAutoStartScript)) {
                Write-Log "No auto-start script found: $monitoringAutoStartScript" "ERROR"
                Add-Content $LogFile "[$timestamp] [ERROR] No auto-start script found" -ErrorAction SilentlyContinue
                return $false
            }
        } else {
            Write-Log "Using unified auto-start script for both RiskNoX and Monitoring agents" "SUCCESS"
            Add-Content $LogFile "[$timestamp] [INFO] Using unified auto-start script for both agents" -ErrorAction SilentlyContinue
        }
        ...
        $startupTriggers = @(
            ...
            # Delayed startup trigger for more reliable boot startup (2 minutes after boot)
            (New-ScheduledTaskTrigger -AtStartup | ForEach-Object { $_.Delay = "PT2M"; $_ }),
            # Additional delayed trigger for network availability (5 minutes after boot)
            (New-ScheduledTaskTrigger -AtStartup | ForEach-Object { $_.Delay = "PT5M"; $_ })
        )
        
        # Enhanced settings for better reliability and system compatibility
        $startupSettings = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -StartWhenAvailable -RestartOnIdle -DontStopOnIdleEnd -ExecutionTimeLimit (New-TimeSpan -Hours 0) -RestartCount 10 -RestartInterval (New-TimeSpan -Minutes 1) -Hidden -MultipleInstances IgnoreNew -WakeToRun -RunOnlyIfNetworkAvailable -Priority 4
        
        # Use current user instead of SYSTEM to avoid directory access issues
        $currentUser = [System.Security.Principal.WindowsIdentity]::GetCurrent().Name
        $startupPrincipal = New-ScheduledTaskPrincipal -UserId $currentUser -LogonType Interactive -RunLevel Highest
        
        $startupTask = New-ScheduledTask -Action $startupAction -Trigger $startupTriggers -Settings $startupSettings -Principal $startupPrincipal -Description "Unified auto-startup for RiskNoX Security Agent, Monitoring Agent and Suricata with retry logic and health monitoring"
        
        Register-ScheduledTask -TaskName $Script:TaskName -InputObject $startupTask -Force | Out-Null
        
        Write-Log "Unified agent startup task created successfully" "SUCCESS"
        Add-Content $LogFile "[$timestamp] [SUCCESS] Unified agent startup task created: $Script:TaskName" -ErrorAction SilentlyContinue
        ...
        Write-Log "Unified auto-startup installation completed successfully" "SUCCESS"
        Add-Content $LogFile "[$timestamp] [SUCCESS] Unified auto-startup installation completed" -ErrorAction SilentlyContinue
        return $true
    }
    catch {
        Write-Log "Error installing unified auto-startup: $_" "ERROR"
        Add-Content $LogFile "[$timestamp] [ERROR] Error installing unified auto-startup: $_" -ErrorAction SilentlyContinue
        return $false
    }
}
        ...
        if ($AgentProcesses) {
            ...
            if ($WorkspaceProcesses.Count -gt 0) {
                ...
                if (!$ProcessRunning) {
                    ...
                    # If process is not accessible, consider it as not running
                    $RunningProcessId = $null
                    $ProcessRunning = $false
                }
...
function Start-MonitoringAgent {
    ...
    try {
        ...
        # Check if agent is truly running or just has a stale PID file
        if ($Status.Running) {
            # Double-check if the process is actually accessible
            $ProcessId = $Status.ProcessId
            $ProcessAccessible = Test-ProcessRunning $ProcessId
            
            if ($ProcessAccessible) {
                Write-Log "Agent is already running and accessible (PID: $ProcessId)" "WARN"
                return $true
            } else {
                Write-Log "Found stale agent process (PID: $ProcessId) - cleaning up before restart" "WARN"
                
                # Force cleanup of stale processes and PID file
                Write-Log "Cleaning up stale processes and PID file..." "INFO"
                
                # Force terminate any stale monitoring-agent processes
                $StaleProcesses = Get-CimInstance -ClassName Win32_Process -Filter "Name = 'monitoring-agent.exe'" -ErrorAction SilentlyContinue
                if ($StaleProcesses) {
                    foreach ($proc in $StaleProcesses) {
                        try {
                            Write-Log "Force terminating stale process PID: $($proc.ProcessId)" "WARN"
                            $proc.Terminate() | Out-Null
                        } catch {
                            Write-Log "Could not terminate process $($proc.ProcessId): $($_.Exception.Message)" "WARN"
                        }
                    }
                    Start-Sleep -Seconds 2
                }
                ...
                # Clean up PID file
                Remove-AgentPid
                Write-Log "Stale process cleanup completed" "INFO"
            }
        }
        ...
        if ($Process) {
            ...
            while ($RetryCount -lt $MaxRetries -and !$AgentStarted) {
                ...
                if ($NewStatus.Running) {
                    ...
                    # Enhanced auto-startup configuration
                    Write-Log "Configuring enhanced auto-startup for monitoring services..." "INFO"
                    ...
                    if (!$autoStartupStatus.AutoStartupEnabled) {
                        ...
                        if ($setupSuccess) {
                            Write-Log "Auto-startup configured successfully" "SUCCESS"
                            ...
                            Write-Log "Warning: Auto-startup installation failed - you may need to run 'setup' command manually" "WARN"
                            Write-Host "⚠ Warning: Auto-startup setup failed - you may need to run the 'setup' command manually after restarts" -ForegroundColor Yellow
                        }
                    } else {
                        Write-Log "Auto-startup already configured, ensuring it's active..." "INFO"
                        Start-AutoStartupTasks | Out-Null
                        Write-Host "✓ Auto-startup is already configured and active" -ForegroundColor Green
                    }
...
function Show-MainMenu {
    ...
    Write-Host "2. Start Agent (Includes Auto-Startup Setup)" -ForegroundColor White
    ...
    if ($args.Count -gt 0) {
        switch ($args[0].ToLower()) {
            ...
            default {
                ...
                Write-Host "  start            - Start the monitoring agent (auto-startup setup included)" -ForegroundColor Gray
                ...
                Write-Host "  setup            - Configure auto-startup only (optional - included in start)" -ForegroundColor Yellow
                Write-Host ""
