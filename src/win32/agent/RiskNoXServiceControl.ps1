...
.DESCRIPTION
    ...
    - Start: Starts service and verifies all processes (backend_server, monitoring_agent, suricata)
...
function Invoke-Start {
    ...
    # Verify all managed processes: backend_server, service_control_backend, monitoring_agent, suricata
    Write-Host ""
    Write-Info "Verifying managed processes..."
    Write-Host ""
    ...
    if (Test-Path $tokenFile) {
        try {
            ...
            if ($response -and $response.processes) {
                $requiredProcesses = @("backend_server", "agent_poll","service_control_backend", "monitoring_agent", "suricata_ids")
...
function Invoke-Stop {
    ...
    Write-Info "This will stop all managed processes (backend_server, service_control_backend, monitoring_agent, suricata)..."
    Write-Host ""
...
function Invoke-Status {
    ...
    Write-Host "    Backend:        logs\backend_server_stdout.log" -ForegroundColor Gray
...
function Show-Help {
    ...
    Write-Host "  • Manages: backend_server, service_control_backend, monitoring_agent, suricata" -ForegroundColor White
    Write-Host ""
