...
$requiredDirs = @($Script:SuricataLog, (Split-Path $Script:SuricataPidFile -Parent), (Split-Path $Script:SuricataLogFile -Parent))
foreach ($dir in $requiredDirs) {
    if (!(Test-Path $dir)) {
        try {
            ...
            # Continue anyway - we'll handle errors when writing files
        }
    }
}
...
function Save-SuricataPid {
    ...
    try {
        ...
        $pidDir = Split-Path $Script:SuricataPidFile -Parent
        if (!(Test-Path $pidDir)) {
            New-Item -ItemType Directory -Path $pidDir -Force -ErrorAction Stop | Out-Null
        }
        
        Set-Content -Path $Script:SuricataPidFile -Value $ProcessId -Force -ErrorAction Stop
...
function Start-SuricataService {
    ...
    try {
        ...
        # Set up environment for Npcap - WORKSPACE ONLY
        $OriginalPath = $env:PATH
        $WorkspaceBin = Split-Path $Script:SuricataBin -Parent
        $WorkspaceNpcap = Join-Path $Script:WorkspacePath "npcap"
        
        # Prioritize workspace npcap FIRST, remove system dependencies
        $env:PATH = "$WorkspaceNpcap;$WorkspaceBin;$OriginalPath"
        
        # Verify workspace npcap files exist
        $RequiredNpcapFiles = @("wpcap.dll", "Packet.dll", "NpcapHelper.exe")
        foreach ($file in $RequiredNpcapFiles) {
            $filePath = Join-Path $WorkspaceNpcap $file
            if (!(Test-Path $filePath)) {
                Write-SuricataLog "Required npcap file missing: $filePath" "ERROR"
                return $false
            }
        }
        
        try {
            # Prepare Suricata arguments - try WinDivert with workspace npcap in PATH
            $SuricataArgs = @(
            ...
            Write-SuricataLog "Using WORKSPACE npcap: $WorkspaceNpcap" "INFO"
            Write-SuricataLog "Workspace npcap prioritized in PATH" "INFO"
            
            # Set additional environment variables to force workspace npcap usage
            $env:NPCAP_DIR = $WorkspaceNpcap
            $env:WPCAP_DLL_DIR = $WorkspaceNpcap
            
            # Pre-load workspace npcap DLLs by copying them to the binary directory temporarily
            $TempDllBackup = @()
            $WorkspaceBinNpcapDlls = @("wpcap.dll", "Packet.dll")
            foreach ($dll in $WorkspaceBinNpcapDlls) {
                $sourcePath = Join-Path $WorkspaceNpcap $dll
                $destPath = Join-Path $WorkspaceBin $dll
                $backupPath = "$destPath.backup.$(Get-Date -Format 'yyyyMMddHHmmss')"
                
                if (Test-Path $destPath) {
                    Copy-Item $destPath $backupPath -Force
                    $TempDllBackup += @{ Original = $destPath; Backup = $backupPath }
                }
                
                if (Test-Path $sourcePath) {
                    Copy-Item $sourcePath $destPath -Force
                    Write-SuricataLog "Copied workspace $dll to binary directory" "INFO"
                }
            }
            ...
            $StartInfo.EnvironmentVariables["NPCAP_DIR"] = $WorkspaceNpcap
            $StartInfo.EnvironmentVariables["WPCAP_DLL_DIR"] = $WorkspaceNpcap
            ...
            # Restore original DLLs if we made backups
            if ($TempDllBackup -and $TempDllBackup.Count -gt 0) {
                Start-Sleep -Seconds 2  # Give process time to load DLLs
                foreach ($backup in $TempDllBackup) {
                    try {
                        if (Test-Path $backup.Backup) {
                            Copy-Item $backup.Backup $backup.Original -Force
                            Remove-Item $backup.Backup -Force
                            Write-SuricataLog "Restored original $(Split-Path $backup.Original -Leaf)" "INFO"
                        }
                    }
                    catch {
                        Write-SuricataLog "Could not restore $(Split-Path $backup.Original -Leaf): $($_.Exception.Message)" "WARN"
                    }
                }
            }
        }
